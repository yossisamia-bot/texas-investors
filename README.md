...
> 🚨 **כלל-ברזל (יוסי, 12/07/2026): את מסמכי המאסטר (`presentation.docx`, `presentation-short.docx`) לא בונים מחדש מסקריפט לעולם - רק עריכה בתוך הקובץ הקיים** (Word ידני, או python-docx כירורגי ברמת run עם exact-match). הקובץ על הדיסק = מקור-האמת; בנייה-מחדש דורסת בשקט עריכות ידניות. שימו לב: טקסט יכול לשבת גם בטבלאות בתוך ה-docx.
